...
iCanvas::iCanvas(IROOTAxisEditor *aE, IROOTFittingObjects *fO, QWidget *parent):QWidget(parent)
{
    ...
    connect(fitObjects, SIGNAL(highLight(QString)), this,SLOT(Highlight(QString)));
...
void iCanvas::CheckAvailableMethods(TObject* obj)
{
    if(!disableMenus){
        if(obj!=NULL){
            ...
            TList *listofMethods = c.GetListOfAllPublicMethods();
            //Fitting
